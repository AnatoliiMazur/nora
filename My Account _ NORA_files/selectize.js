	...
	return e.mixin(J), c.mixin(J), a.extend(J.prototype, {
		...
		search: function(b) {
			...
			if (b !== f.lastQuery ? (f.lastQuery = b, d = f.sifter.search(b, a.extend(h, {
				score: e
			})), f.currentResults = d) : d = a.extend(!0, {}, f.currentResults), g.hideSelected) for (c = d.items.length - 1; c >= 0; c--) - 1 !== f.items.indexOf(w(d.items[c].id)) && d.items.splice(c, 1);
			return d
		},
		refreshOptions: function(b) {
			...
			if (x.html(n.join("")), t.settings.highlight && v.query.length && v.tokens.length) for (c = 0, g = v.tokens.length; g > c; c++) d(x, v.tokens[c].regex);
		...
		getElementWithValue: function(b, c) {
			if (b = w(b)) for (var d = 0, e = c.length; e > d; d++) if (c[d].getAttribute("data-value") === b) return a(c[d]);
			return a()
		},
		getItem: function(a) {
		...
		open: function() {
		    //$("#field_2_3 .ginput_container_select").click(function(){
            //    $('#field_2_3 .selectize-control').addClass('selectize-control-height');
            //}); 
			var a = this;
		...
		close: function() {
		    //$('#field_2_3 .selectize-control').removeClass('selectize-control-height'); 
			var a = this,
				b = a.isOpen;
